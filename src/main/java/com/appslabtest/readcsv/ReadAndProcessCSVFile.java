...
public class ReadAndProcessCSVFile {
	...
	public CSVParser getCSVParser(File file) {
		...
		try {
			...
			if(!(file.getName().contains("CONV"))) {
				...
				csvFormat = CSVFormat.EXCEL.withSkipHeaderRecord(false);
				System.out.println("Header not is skipped");
			}
	...
	public void writeAllRecords(File file, String validatingMonth) {
		...
		if(! getRecords_In_OWD_Not_In_EDW().isEmpty()) {
			File records_In_OWD_Not_In_EDW = new File(directiroryName + "/" + filename.concat("_records_In_OWD_Not_In_EDW.csv"));
		...
		if(! getRecords_In_EWD_And_In_ODW().isEmpty()) {
			File records_In_EWD_And_In_ODW = new File(directiroryName + "/" + filename.concat("_records_In_EWD_And_In_ODW.csv"));
		...
		if(! getRecords_In_EWD_Not_In_ODW().isEmpty()) {
			File records_In_EWD_Not_In_ODW = new File(directiroryName + "/" + filename.concat("_records_In_EWD_Not_In_ODW.csv"));
		...
		if(! getRecords_In_OWD_And_In_EDW().isEmpty()) {
			File records_In_OWD_And_In_EDW = new File(directiroryName + "/" + filename.concat("_records_In_OWD_And_In_EDW.csv"));
